...
- Compute initial condition of the WTD.
4. Calibrate the peat hydraulic properties.
5. Run the model

## Prepare the peat hydrology domain. Meshing.
...
1. Vectorize raster area into a single polygon, as described in [[Vectorize raster into single polygon]].
2. Create a mesh
...
Note:
- With FiPy 3.4.2.1 and Gmsh 4.8.4, the default exporting or saving options of gmsh end up in a .msh file that fipy does not understand. 
Solution: Open the .msh in gmsh, File>Export>(choose .msh filename) > Save in Version 2 ASCII. Version 4 is the one FiPy does not understand.
- Leave the options 'Svae all elements' and ' parametric blablabla' unselected
Important if file is going to be read by fipy: [[Import a gmsh mesh into FiPy]]

Update: At this point we can import it into the python script (no need of Step 3)
...
4. Take the output of Step 2 and create the NetworkX graph that will be used during the simus. Including the canal blocks.
...
Creates a pickle file that is the input for the main program.

#### Additional details
The details of what happens inside these functions are the follwoing:
